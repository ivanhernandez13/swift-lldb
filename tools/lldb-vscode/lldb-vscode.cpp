...
//----------------------------------------------------------------------
// NOTE: This handlers hijacks launch request for remote_launch.
// "LaunchRequest": {
//   "allOf": [ { "$ref": "#/definitions/Request" }, {
//     "type": "object",
...
void request_launch_remote(const llvm::json::Object &request) {
  ...
  //    g_vsc.exit_commands = GetStrings(arguments, "exitCommands");
  //    g_vsc.stop_at_entry = GetBoolean(arguments, "stopOnEntry", false);
  const auto port = GetUnsigned(arguments, "port", -1);
  //    const auto debuggerRoot = GetString(arguments, "debuggerRoot");
  ...
  llvm::StringRef remote_program = GetString(arguments, "remote_program");
...
//  *g_vsc.log << "Connecting to remote debug server on port " << port << std::endl;
  ...
  g_vsc.debugger.SetAsync(false);
  auto process = g_vsc.target.ConnectRemote(listener, hostURL.c_str(), nullptr, error);
  g_vsc.debugger.SetAsync(true);
...
//      *g_vsc.log << platform.GetOSDescription() << std::endl;
  ...
  g_vsc.launch_info.SetLaunchFlags(14u);
...
//      *g_vsc.log << "Remote launching!" << std::endl;
      ...
      //                                 MakeArgv(args).data(),
      //                                 MakeArgv(envs).data(),
      //                                 nullptr,nullptr,nullptr,nullptr, // in,out,err,cwd
      //                                 flags,
      //                                 false,
      //                                 error)) {
...
void request_launch(const llvm::json::Object &request) {
  auto arguments = request.getObject("arguments");
  if (ObjectContainsKey(*arguments, "port")) {
